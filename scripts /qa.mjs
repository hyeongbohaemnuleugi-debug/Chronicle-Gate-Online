...
for(const c of CAMPAIGNS){
  ...
  for(const beat of c.storyBeats||[]){
    ...
    ok(beat.freeActionAllowed===true,`${c.id}/${beat.id}: free action disabled`);
...
ok(/storyActionSubmitBtn'\)\.onclick=submitStoryDeclaration/.test(client),'free-action submit button is not wired');
ok(/freeActionAllowed:true/.test(server),'parallel free action is not enabled');
ok(/validateFreeAction\(declaration,scene\)/.test(server),'parallel free action is not validated');
ok(fs.existsSync(path.join(root,'supabase/migrations/202609010002_accounts_and_progress.sql')),'account migration missing');

if(errors.length){
  console.error(`Chronicle Gate v9 QA failed (${errors.length})`);
  for(const e of errors.slice(0,80)) console.error(` - ${e}`);
  process.exit(1);
}
console.log(`Chronicle Gate v9 QA OK: ${CAMPAIGNS.length} chronicles, ${CAMPAIGNS.reduce((n,c)=>n+(c.storyBeats?.length||0),0)} canonical scenes.`);

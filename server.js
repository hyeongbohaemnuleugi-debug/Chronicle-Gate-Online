...
async function recordEnding(accountId, { endingKey, campaignId, endingTitle }) {
...
async function buyDice(accountId, diceId, price) {
  if (!accountId) throw new Error('로그인이 필요합니다.');
  const existingAccount = await findAccountById(accountId);
  if (!existingAccount) throw new Error('계정을 찾을 수 없습니다.');
  if (isDeveloperEmail(existingAccount.email)) return publicAccount(existingAccount);
  if (!accountPersistenceEnabled) {
...
async function equipDice(accountId, diceId) {
  const account = await findAccountById(accountId);
  if (!account) throw new Error('계정을 찾을 수 없습니다.');
  const developer = isDeveloperEmail(account.email);
  const owned = developer ? ALL_DICE_IDS : (Array.isArray(account.owned_dice) ? account.owned_dice : ['classic']);

...
export class DiceTheater {
  ...
  makeLabel(text, size = .34) {
    ...
    const x = c.getContext('2d'); x.clearRect(0, 0, 256, 256); x.textAlign = 'center'; x.textBaseline = 'middle'; x.font = '800 116px Georgia'; x.shadowColor = 'rgba(0,0,0,.9)'; x.shadowBlur = 15; x.fillStyle = '#fff9ec'; x.fillText(text, 128, 138);
  ...
  d20(color) {
    ...
    for (let f = 0; f < 20; f++) {
      ...
      const mat = new THREE.MeshPhysicalMaterial({ color: tint, roughness: .24, metalness: .36, clearcoat: 1, clearcoatRoughness: .12, emissive: 0x000000, emissiveIntensity: 0 });
      ...
      const label = this.makeLabel(String(f + 1), .44); label.position.copy(center.clone().add(normal.clone().multiplyScalar(.022))); label.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), normal); group.add(label); this.labelMaterials[f] = label.userData.material;
    }
  ...
  d6(color) {
    ...
    faces.forEach(({ normal, roll }, idx) => {
      const tint = new THREE.Color(color || '#b94836').offsetHSL(0, 0, (idx % 3 - 1) * .025);
      const mat = new THREE.MeshPhysicalMaterial({ color: tint, roughness: .22, metalness: .32, clearcoat: 1, clearcoatRoughness: .1, emissive: 0x000000, emissiveIntensity: 0 });
    ...
    faces.forEach(({ normal, roll }) => {
      ...
      const label = this.makeLabel(String(roll), .72); label.position.copy(v.clone().multiplyScalar(1.136)); label.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), v); group.add(label); this.labelMaterials[roll - 1] = label.userData.material;
    });
  ...
  async roll({ sides = 20, result = 1, color = '#b94d36', duration = 2600 }) {
    ...
    const die = sides === 6 ? this.d6(color) : this.d20(color);

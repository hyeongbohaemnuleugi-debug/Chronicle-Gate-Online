...
function renderAccountBar(){
  ...
  $('#accountStatus').textContent=logged?`${account.displayName}${account.accountCode?` · #${account.accountCode}`:''} · ${account.email}`:'로그인하면 이어하기·도감·보상을 사용할 수 있습니다.';
...
function enqueueDice(payload) {
  diceQueue = diceQueue.then(async () => {
    ...
    if (startsAt > Date.now()) await new Promise(r => setTimeout(r, Math.min(20, startsAt - Date.now())));
    $('#diceSub').textContent = '주사위가 테이블 위를 구릅니다…';
    if (theater) {
      try {
        await theater.roll({ sides: payload.sides, result: payload.result, color: c?.accent || '#bf4a38', skin: payload.diceSkin || null, duration: payload.sides === 20 ? 1540 : 1320 });
      } catch (error) {
...
function renderParallelStory() {
  ...
  if (ps.ended) {
    ...
    $('#storyPrompt').innerHTML = '<strong>당신의 이야기는 여기서 멈춥니다.</strong><span>다른 플레이어들의 선택은 계속 지켜볼 수 있습니다.</span>';
